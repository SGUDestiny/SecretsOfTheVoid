...
public class AbyssalithBacktankItem extends AbyssalithGearItem implements IBacktank {
    public AbyssalithBacktankItem(ArmorMaterial pMaterial, Type pType, Properties pProperties)
    {
    ...
    public void onCraftedBy(ItemStack stack, Level level, Player player) {
        if (stack.getTag() != null && stack.getTag().getDouble(MAX_OXYGEN) != 1620) {
            stack.getOrCreateTag().putDouble(MAX_OXYGEN, 1620);
            stack.getOrCreateTag().putDouble(STORED_OXYGEN, 1620);
        }
    }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level pLevel, List<Component> components, TooltipFlag flag) {
        if(!stack.getTag().isEmpty()) {
            MutableComponent oxygen = Component.translatable("tooltip.secretsofthevoid.backtank_oxygen").withStyle(ChatFormatting.BLUE);
